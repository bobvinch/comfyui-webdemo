...
import {useAuthUseregisterWithPassword} from "~/composables/authhooks";
...
const handleRegister = async () => {
  ...
  if (valid) {
    ...
    const {status,msg,data:user}=await useAuthUseregisterWithPassword(username.value,email.value, password.value);
    if(status===0){
      // 注册成功
      useSnackbarStore().showSuccessMessage(msg)
      useAuthStore().user=user
      await useAuthautoLogin()
    }else{
